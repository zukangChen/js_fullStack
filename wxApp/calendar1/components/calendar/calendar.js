  ...
  methods: {
    ...
    lastMonth(){
      ...
      createEmptyGrids(year,month){
        let week = new Date(Date.UTC(year,month -1,1)).getDay(),//获取当前年月的星期几
        emptyGridsBefore = [],
        emptyGridsAfter = [],
        emptyDays = (week == 0 ? 7 : week);
        let thisMonthDays = this.getThisMonthDays(year,month)//获取当月天数
        let preMonthDays = month -1 <0 ? this.getThisMonthDays(year-1,12) : this.getThisMonthDays(year,month-1)//获取上月的天数
